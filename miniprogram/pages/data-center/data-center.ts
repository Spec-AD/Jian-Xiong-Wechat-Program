...
Page({
  data: {
    ...
    likesStats: {
      total: 0,
      trend: [] as StatItem[],
      topWorks: [] as { title: string; count: number }[],
    },
  ...
  async _loadWeekStats() {
    try {
      const data = await request<{
        ...
        weekLikes: number
        weekViews: number
      }>({ url: '/user/stats/week', method: 'GET' })
      this.setData({
  ...
  async _loadPublishStats() {
    try {
      const data = await request<{
        ...
        trend: StatItem[]
      }>({ url: '/user/stats/publish', method: 'GET' })
      this.setData({
  ...
  async _loadLikesStats() {
    try {
      const data = await request<{
        total: number
        trend: StatItem[]
        topWorks: { title: string; count: number }[]
      }>({ url: '/user/stats/likes', method: 'GET' })
      this.setData({
  ...
  async _loadViewsStats() {
    try {
      const data = await request<{
        ...
        trend: StatItem[]
        topWorks: { title: string; count: number }[]
      }>({ url: '/user/stats/views', method: 'GET' })
      this.setData({
