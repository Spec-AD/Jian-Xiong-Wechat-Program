...
Page({
  data: {
    ...
    keyword: '',
    // 活跃 tab
    activeTab: 'works' as 'works' | 'stats' | 'cos-import',
    // COS 资源导入
    cosObjects: [] as CosObjectInfo[],
    cosLoading: false,
    cosTotalFiles: 0,
    cosStats: null as any,
    cosSelectedKeys: [] as string[],
    cosImporting: false,
    cosImportResult: null as any,
    cosFilterType: 'all' as string,
    cosSearchText: '' as string,
    cosSelectAll: false,
    filteredCosObjects: [] as CosObjectInfo[],
  },
  ...
  /** 切换tab */
  onTabChange(e: any) {
    const tab = e.currentTarget.dataset.tab as 'works' | 'stats' | 'cos-import'
  ...
  async onLoadCosResources() {
    ...
    try {
      // 1. 先获取统计信息
      const stats = await request<any>({ url: '/cos/stats', method: 'GET' })
      
      // 2. 获取资源列表（最多 1000 个）
      let allObjects: CosObjectInfo[] = []
  ...
  /** 获取过滤后的对象列表 */
  _getFilteredObjects(): CosObjectInfo[] {
  ...
  async onCosImport() {
    ...
    try {
      ...
      toast(`成功导入 ${result.imported} 个资源`, 'success')
    } catch (err: any) {
      wx.hideLoading()
  ...
  /** 获取文件类型对应的显示名 */
  _getExtLabel(ext: string): string {
