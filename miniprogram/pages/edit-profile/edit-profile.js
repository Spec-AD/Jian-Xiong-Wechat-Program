...
Page({
    ...
    async _loadProfile() {
        try {
            const profileData = await (0, api_1.request)({
                url: '/user/profile',
                method: 'GET',
            });
            this.setData({
    ...
    async _uploadAvatar(tempPath) {
        ...
        try {
            const result = await (0, api_1.uploadFile)(tempPath, 'image', 'avatar');
            this.setData({ 'profile.avatarUrl': result.url });
            // 同时更新后端用户信息
            await (0, api_1.updateUserProfile)({ avatarUrl: result.url });
            // 更新全局
            app.saveUserInfo({
    ...
    async onSave() {
        ...
        try {
            await (0, api_1.updateUserProfile)({
