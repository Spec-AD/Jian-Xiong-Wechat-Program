...
exports.splitStreamContent = splitStreamContent;
/**
 * 将纯文本中的 HTML 特殊字符转义
 */
...
function parseInline(text) {
    ...
    // 链接 [text](url)
    result = result.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" style="color:#6A005F;text-decoration:underline;">$1</a>');
    return result;
}
/**
...
function markdownToHtml(md) {
    ...
    function flushCodeBlock() {
        if (codeContent.length > 0) {
            ...
            htmlLines.push(`<pre style="background:#1e1e1e;color:#d4d4d4;padding:24rpx 28rpx;border-radius:12rpx;font-size:24rpx;line-height:1.6;font-family:Menlo,monospace;white-space:pre-wrap;word-break:break-all;overflow-x:auto;max-width:100%;"><code${langClass}>${escapeHtml(codeContent.join('\n'))}</code></pre>`);
            codeContent = [];
            codeLang = '';
        }
    }
    function flushQuote() {
        if (quoteLines.length > 0) {
            htmlLines.push(`<blockquote style="border-left:6rpx solid #6A005F;padding:16rpx 20rpx;margin:16rpx 0;background:#f9f0f7;color:#555;border-radius:4rpx;">${quoteLines.join('<br/>')}</blockquote>`);
            quoteLines = [];
        }
    }
    function flushList() {
    ...
    function flushLaTeXBlock() {
        if (latexLines.length > 0) {
            htmlLines.push(`<div style="background:#f8f9fa;padding:20rpx 24rpx;margin:12rpx 0;border-radius:8rpx;font-family:serif;font-style:italic;text-align:center;color:#2c3e50;border:1rpx solid #e8e8e8;overflow-x:auto;"><text>$$${latexLines.join('\n')}$$</text></div>`);
            latexLines = [];
        }
    }
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headingMatch) {
            ...
            htmlLines.push(`<h${level} style="font-size:${sizes[level] || '28rpx'};font-weight:${fontWeight};margin:20rpx 0 8rpx;color:#333;line-height:1.4;">${content}</h${level}>`);
            continue;
        }
        // ===== 引用块 =====
        if (trimmed.startsWith('> ')) {
